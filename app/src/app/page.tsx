...
import Spinner from '@/components/Spinner'; // 1. Spinner 임포트
...
export default function Home() {
  ...
  const [isLoading, setIsLoading] = useState(true); // 온보딩/초기 로딩 상태
  const [isApiLoading, setIsApiLoading] = useState(true); // API 데이터 로딩 상태
  ...
  // 전체 로딩 상태 정의 (온보딩 확인 중이거나 API 호출 중일 때)
  const showFullSpinner = isLoading || isApiLoading;

  return (
    <>
      {/* 2. 스피너 적용 (화면 전체 390px 중앙 오버레이) */}
      <Spinner loading={showFullSpinner} />
      ...
      <VStack
        $css={{
          ...
          // 로딩 중에는 콘텐츠 숨김 처리 (선택 사항)
          visibility: showFullSpinner ? 'hidden' : 'visible',
        }}
      >
        <header>
          <HStack
                ...
                {accommodations.map((s: any) => {
                  ...
                  return (
                    <SamchonCard
                      ...
                      onClick={() =>
                        router.push(`/reservation/detail/${s.accommodationId}`)
                      }
                    />
                  );
                })}
